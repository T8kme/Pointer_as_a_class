...
void TForm1::Draw_Shield() {
	...
	for (int i = 6; i < 60; i++) { // shield inside
		TColor color1 = RGB(0, 70 + i + pointer1.shield - 6, 111 + i + pointer1.shield - 6);
	...
	bitmap1->Canvas->TextOut(center_pos - (IntToStr(pointer1.value).Length() +
		3) / 2 * 12, 180, IntToStr(pointer1.value) + "µm");
	...
	for (int i = (-size/(10*5))+(size/2); i < (size/(10*5))+(size/2); i++) { // center point
		bitmap1->Canvas->Pen->Color = clBlack;
		bitmap1->Canvas->Brush->Color = pointer1.color2;
		bitmap1->Canvas->Ellipse(i, i, size - i, size - i);
	}
	...
	if (pointer1.out_of_range) {
		...
		bitmap1->Canvas->TextOut(size / 2 - 7  * 12, size - size/4, "OUT OF RANGE!");
...
void __fastcall TForm1::Timer1Timer(TObject *Sender) {
	if (pointer1.run) {
		...
		if (pointer1.target > pointer1.max_value && pointer1.value == pointer1.max_value)
